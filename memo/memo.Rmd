...
ggsave("../plots/plot-3.jpg", width = 10, height = 4)
```
...
overweight_support_long <- data |>
  pivot_longer(cols = c(Overweight, FamilySupport), 
               names_to = "Category", values_to = "Percentage")

overweight_support_long |>
  ggplot(aes(x = Age, y = Percentage, fill = Category)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) +
...
ggsave("../plots/plot-4.jpg", width = 14, height = 10)
```
...
```{r}
drunk_smoke_meals_data <- drunk_smoke_meals_data |>
  select(
    Age = `Age.x`
  ...
  drop_na() |>
  mutate(Age = factor(Age, levels = c("14 and younger", "15.0", "16.0", "17.0", "18 and older"))) |>
  pivot_longer(
    cols = Age,
    names_to = "Category",
    values_to = "Count"
...
```{r}
ggplot(drunk_smoke_meals_data, aes(x = Category, y = Count, fill = Category)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
